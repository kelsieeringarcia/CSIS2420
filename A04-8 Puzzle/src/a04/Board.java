...
/**
 * This class is for the board for the 8 puzzle. 
 * Constructing the board and checking information of the current puzzle state.
 * 
...
public class Board {
	...
	/**
	 * This method goes through the board array to verify how many blocks
	 * are out of place
	...
	/**
	 * This method adds up the amount of moves it will take to get in final position.
	 * 
...
//			System.out.println(String.format("i:%d col:%d row:%d", i+1, i%N, i/N));
			double rowDifference = Math.abs( 
					Math.ceil((double) board[i]/(double) N) - 
					Math.ceil((double) (i+1)/(double) N) );
			int colDifference = Math.abs( 
					(board[i]%N == 0 ? N : board[i]%N) - 
					((i+1)%N == 0 ? N : (i+1)%N) );
	...
	/**
	 * This method checks to see if the square is solvable
	 * this checks for even or odd sized square.
	...
	public int getBlankRow() {
		double res = Math.ceil((double) Arrays.asList(board).indexOf(0) / (double) N) - 1;
		return (int) res;
	}
	...
	/**
	 * This method checks the neighboring tiles of each tile and 
	 * keeps track of the empty block.
	 * @return 
	 */
